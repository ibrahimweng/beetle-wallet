...
import {
  AgentAsk, AgentSay, AmountPad, Banner, BigStatus, Button, ChoiceRow, Choices, Dock,
  Facts, FootNote, Label, PageHead, ReasonList, Screen, ToolPanel, colour,
} from '../design';
...
export const Checking = ({ nav }: { nav: Nav }) => (
  <Screen dock={dock('Ask how I decide', nav)}>
    ...
    <ToolPanel tool="Beetle Reasoning" state="Checked" rows={[
    ...
    <ReasonList title="How I decided" rows={[
    ...
    <Choices>
      <ChoiceRow glyph="chat" title="It is ₦20,000" sub="Rent, the usual amount" onPress={() => nav.go('confirm')} />
      <ChoiceRow glyph="list" title="Let me type it" sub="I would rather you set this one" onPress={() => nav.go('typed')} />
    </Choices>
  </Screen>
);
...
export const IWillNot = ({ nav }: { nav: Nav }) => (
  <Screen dock={dock('Ask why I stopped this', nav)}>
    ...
    <BigStatus glyph="warn-filled" tone={colour.good} amount="₦595,320" line="to an account I have never seen" />
    <Facts rows={[['You said', 'send everything'], ['Account age', 'Four minutes'], ['Paid before', 'Never']]} />
    <AgentSay>Your whole balance, to an account four minutes old.</AgentSay>
    <Choices>
      <ChoiceRow glyph="pot" title="Send ₦20,000 instead" sub="Enough to check it arrives" onPress={() => nav.go('pay')} />
      <ChoiceRow glyph="send" title="Wait until tomorrow" sub="I will ask you again then" onPress={() => nav.go('home')} />
      <ChoiceRow glyph="request" title="It really is me" sub="Face ID, then a call from us" onPress={() => nav.go('confirm')} />
    </Choices>
...
export const Misheard = ({ nav }: { nav: Nav }) => (
  <Screen dock={dock('Ask me about this', nav)}>
    ...
    <BigStatus glyph="warn-filled" tone={colour.warn} amount="₦200,000" line="and I am not sure I heard it right" />
    <Facts rows={[['You said', 'two hundred'], ['I heard', '₦200,000'], ['Or maybe', '₦200']]} />
    <AgentSay>Spoken round numbers are where I slip most. I will not choose for you.</AgentSay>
    <Choices>
      <ChoiceRow glyph="pot" title="It is ₦200,000" sub="Rent money, to Sarah" onPress={() => nav.go('pay')} />
      <ChoiceRow glyph="send" title="It is ₦200" sub="Small change, to Sarah" onPress={() => nav.go('pay')} />
      <ChoiceRow glyph="request" title="Let me type it" sub="Neither one is right" onPress={() => nav.go('amend')} />
    </Choices>
...
export const AlreadyGone = ({ nav }: { nav: Nav }) => (
  <Screen dock={dock('Ask about the cover', nav)}>
    ...
    <AgentSay>You said two hundred. I sent two hundred thousand. That is mine to put right, not yours.</AgentSay>
    <Choices>
      <ChoiceRow glyph="send" title="Take ₦199,800 back" sub="Paid by us today, not in days" onPress={() => nav.go('reversed')} />
      <ChoiceRow glyph="bank" title="Ask Sarah to return it" sub="We do this to recover our side" onPress={() => nav.go('recall')} />
    </Choices>
    <AgentAsk question="Want to know how I stop this?" answer="Tell me" onAnswer={() => nav.go('agentchat')} />
  </Screen>
);
...
export const Short = ({ nav }: { nav: Nav }) => (
  <Screen dock={dock('Ask me about this', nav)}>
    ...
    <BigStatus glyph="warn-filled" tone={colour.warn} amount="₦7,520" line="short of the ₦20,000 you asked for" />
    <Facts rows={[['You asked for', '₦20,000'], ['In Everyday', '₦12,480'], ['Short by', '₦7,520']]} />
    <AgentSay>Three ways to close it. None of them costs you anything.</AgentSay>
    <Choices>
      <ChoiceRow glyph="pot" title="Move it from Holiday" sub="₦48,000 is sitting there" onPress={() => nav.go('goal')} />
      <ChoiceRow glyph="send" title="Send ₦12,480 now" sub="The rest when your salary lands" onPress={() => nav.go('pay')} />
      <ChoiceRow glyph="request" title="Ask Musa for ₦7,520" sub="He owes you from the rent" onPress={() => nav.go('askreq')} />
    </Choices>
    <FootNote title="Nothing has left your account" sub="No fee and no attempt. This is a sum I did before trying." />
  </Screen>
);
...
export const Pending = ({ nav }: { nav: Nav }) => (
  <Screen dock={dock('Ask about this transfer', nav)}>
    ...
    <ToolPanel tool="Beetle Transfers" state="Waiting" rows={[
      { k: 'Left your account', v: '14:22' },
      { k: 'GTBank has it', v: '14:22' },
      { k: 'Reaching Sarah', v: 'Waiting', done: false },
    ]} />
    <AgentSay>Slow, not lost. If GTBank has not confirmed by 16:22 it comes back on its own, and I will tell you either way.</AgentSay>
    <AgentAsk question="Want a message the moment it lands?" answer="Yes, tell me" onAnswer={() => nav.go('home')} />
  </Screen>
);
...
export const Failed = ({ nav }: { nav: Nav }) => (
  <Screen dock={dock('Ask why this failed', nav)}>
    ...
    <AgentSay>Nothing was taken and nothing was charged. GTBank has been failing since 13:40, so this is their afternoon, not your account.</AgentSay>
    <Choices>
      <ChoiceRow glyph="send" title="Try again now" sub="It may have cleared already" onPress={() => nav.go('confirm')} />
      <ChoiceRow glyph="bank" title="Send it another way" sub="Through your Zenith account" onPress={() => nav.go('payfrom')} />
    </Choices>
...
export const Reversed = ({ nav }: { nav: Nav }) => (
  <Screen dock={dock('Ask about this', nav)}>
    ...
    <Facts rows={[
      ['Left', '14:22'], ['Came back', '16:22'],
      ['Why', 'Account could not be credited'], ['Reference', 'REV-40118-2290'],
    ]} />
    <AgentSay>Sarah never got it, so GTBank sent it back and I put it where it came from. Nothing was charged.</AgentSay>
    <Choices>
      <ChoiceRow glyph="search" title="Check the account number" sub="One digit is usually all it is" onPress={() => nav.go('pay')} />
      <ChoiceRow glyph="send" title="Try Sarah again" sub="Same amount, same account" onPress={() => nav.go('confirm')} />
    </Choices>
  </Screen>
);
...
export const Wrong = ({ nav }: { nav: Nav }) => (
  <Screen dock={dock('Ask what you can do', nav)}>
    <PageHead title="What went wrong?" sub="₦20,000 to Sarah Adeyemi, 14:22" />
    <AgentSay>Some of this I can do in minutes. Some of it only a bank can do, and that takes days. I will tell you which one you are in before you start, not after.</AgentSay>
    <Choices>
      <ChoiceRow glyph="person" title="It went to the wrong person" sub="I ask their bank to send it back" onPress={() => nav.go('recall')} />
      <ChoiceRow glyph="search" title="They say it never arrived" sub="I make GTBank trace it" onPress={() => nav.go('pending')} />
      <ChoiceRow glyph="shield" title="I did not make this payment" sub="I freeze the account first, then we look" onPress={() => nav.go('disputeopen')} />
    </Choices>
  </Screen>
);
...
export const Recall = ({ nav }: { nav: Nav }) => (
  <Screen dock={dock('Ask what happens next', nav)}>
    ...
    <ToolPanel tool="Beetle Recall" state="Running" rows={[
    ...
    <ReasonList title="What this is and is not" rows={[
    ...
    <Choices>
      <ChoiceRow glyph="chat" title="Message Sarah" sub="Most of these end here, in an hour" onPress={() => nav.go('agentchat')} />
      <ChoiceRow glyph="list" title="Open a dispute" sub="If she has not answered by Friday" onPress={() => nav.go('disputeopen')} />
    </Choices>
  </Screen>
);
...
export const DisputeOpen = ({ nav }: { nav: Nav }) => (
  <Screen dock={dock('Ask where this stands', nav)}>
    ...
    <ToolPanel tool="Beetle Dispute" state="Day 3" rows={[
    ...
    <ReasonList title="Where this actually is" rows={[
    ...
    <Choices>
      <ChoiceRow glyph="chat" title="See what was filed" sub="The exact wording, and what was attached" onPress={() => nav.go('agentchat')} />
      <ChoiceRow glyph="list" title="Add something to it" sub="A screenshot or a message that helps" onPress={() => nav.go('agentchat')} />
    </Choices>
  </Screen>
);
...
export const DisputeEnd = ({ nav }: { nav: Nav }) => (
  <Screen dock={dock('Ask about this dispute', nav)}>
    ...
    <ToolPanel tool="Beetle Dispute" state="Closed" rows={[
      { k: 'You reported it', v: '28 Aug' },
      { k: 'GTBank decided', v: '3 Sep' },
      { k: 'Money returned', v: '11:40', done: false },
    ]} />
    <AgentSay>Six days, and you did not chase it once. Most disputes that get this far end the same way.</AgentSay>
    <AgentAsk question="Want the closing letter for your records?" answer="Save it" onAnswer={() => nav.go('history')} />
  </Screen>
);
...
export const NoNetwork = ({ nav }: { nav: Nav }) => (
  <Screen dock={dock('Ask what works offline', nav)}>
    ...
    <AgentSay>I will not send money against a balance I cannot check. Tell me what you want and it goes the moment you are back.</AgentSay>
    <Choices>
      <ChoiceRow glyph="send" title="Queue it for later" sub="Waits here until I can check" onPress={() => nav.go('home')} />
      <ChoiceRow glyph="bank" title="Pay by USSD instead" sub="Works with no data at all" onPress={() => nav.go('home')} />
    </Choices>
    <AgentAsk question="Turn on lite mode while data is short?" answer="Turn it on" onAnswer={() => nav.go('settings')} />
  </Screen>
);
...
export const Amend = ({ nav }: { nav: Nav }) => {
  ...
  return (
    <Screen dock={undefined}>
      ...
      <AgentSay>Change it as many times as you like. It moves after your face and your passcode, not before.</AgentSay>
