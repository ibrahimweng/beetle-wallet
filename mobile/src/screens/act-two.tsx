...
import {
  AgentSay, Aside, Banner, BigStatus, BottomBar, Button, CapRow, Card, ChoiceRow, Choices,
  DeviceRow, Dock, Facts, FootNote, Icon, PageHead, Screen, SectionLabel, SettingRow,
  ToggleRow, Usage, Caption, Head, Label, Meta, Row, colour, space,
} from '../design';
import { Route } from '../routes';
import { get, update } from '../state/store.js';
...
export const Rule = ({ nav }: { nav: Nav }) => (
  <Screen dock={undefined}>
    ...
    <FootNote title="You can stop it any time" sub="It sits in Standing instructions with a switch beside it." />
  ...
  const instruction = (title: string, when: string, log: string) => (
    <Card key={title} style={{ gap: space.s3 }}>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center' }}>
        <Meta tone="secondary" style={{ flex: 1 }}>{log}</Meta>
        <Pressable accessibilityRole="button" onPress={() => nav.go('history')}><Label>See log</Label></Pressable>
      </View>
    </Card>
  );
  return (
    <Screen dock={dock('Ask me to set one up', nav)}>
      ...
      <Card style={{ gap: space.s3 }}>
        <ToggleRow title="Money is tight this month" value={tight} onChange={setTight}
          sub="Turn this on and I stop moving money into savings, and ask before anything that is not a bill." />
        <Caption tone="secondary">You can also just tell me, any time.</Caption>
      </Card>
      {instruction('Move ₦20,000 to Holiday on payday', 'The day your salary lands.', 'Moved 4 times · ₦80,000 put aside')}
...
export const Settings = ({ nav }: { nav: Nav }) => (
  <Screen dock={dock('Ask me to change something', nav)}>
    ...
    <View style={{ gap: 32 }}>
      <SettingRow glyph="faceid-filled" title="Lock and privacy" value="Face ID" onPress={() => nav.go('lock')} />
      <SettingRow glyph="shield-filled" title="Spending limits" value="₦100,000 a day" onPress={() => nav.go('limits')} />
      <SettingRow title="Standing instructions" value="3 running" onPress={() => nav.go('rules')} />
      <SettingRow glyph="laptop-filled" title="Devices" value="3 signed in" onPress={() => nav.go('devices')} />
      <SettingRow glyph="key-filled" title="Keys and recovery" value="Set up" onPress={() => nav.go('newcode')} />
    </View>
    ...
    <Meta tone="tertiary" style={{ textAlign: 'center' }}>Version 1.0.4</Meta>
  </Screen>
);
...
export const Lock = ({ nav }: { nav: Nav }) => {
  ...
  return (
    <Screen dock={dock('Ask me to lock something down', nav)}>
      ...
      <View style={{ gap: 32 }}>
        <ToggleRow glyph="faceid-filled" title="Face ID" value={face} onChange={setFace} />
        <SettingRow glyph="key-filled" title="Passcode" value="6 digits" onPress={() => nav.go('newcode')} />
        <SettingRow glyph="clock-filled" title="Ask again after" value="2 minutes" onPress={() => nav.go('settings')} />
      </View>
      <Head>What other people can see</Head>
      <View style={{ gap: 32 }}>
        <ToggleRow glyph="eye-filled" title="Hide my balance" value={hide} onChange={setHide} />
        <ToggleRow glyph="camera-filled" title="Hide it in screenshots" value={shots} onChange={setShots} />
        <ToggleRow title="Amounts in notifications" value={amounts} onChange={setAmounts} />
      </View>
      <Aside glyph="eye">With this on, your balance is dots until you look at it.</Aside>
      <FootNote title="Your passcode is not on our servers"
...
export const Limits = ({ nav }: { nav: Nav }) => (
  <Screen dock={dock('Ask me to change a limit', nav)}>
    ...
    <Usage out="₦64,000" of="₦100,000" note="₦36,000 left before I stop and ask you twice." />
    <Head>Your caps</Head>
    <View style={{ gap: 32 }}>
      <CapRow title="One transfer" sub="The most that can leave in a single go" value="₦50,000" onPress={() => nav.go('limitstop')} />
      <CapRow title="One day" sub="Midnight to midnight" value="₦100,000" onPress={() => nav.go('limitstop')} />
      <CapRow title="One month" sub="Resets on the first" value="₦900,000" onPress={() => nav.go('limitstop')} />
    </View>
    ...
    <Meta tone="secondary">Two deliberate things, so a bad minute cannot carry you past your own rule.</Meta>
    <Pressable accessibilityRole="button" onPress={() => nav.go('limitstop')}
      style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
      <Row>Show me what that looks like</Row>
      <Icon name="chevron" size={20} />
    </Pressable>
    <Aside glyph="clock">Raising a cap takes a day to come into force. Lowering one is immediate.</Aside>
  </Screen>
);
...
export const LimitStop = ({ nav }: { nav: Nav }) => {
  ...
  return (
    <Screen dock={<BottomBar onBack={nav.back}><Button label="Send ₦120,000" disabled={left > 0} onPress={() => nav.go('confirm')} /></BottomBar>}>
      <PageHead title="Past your own limit" sub="Nothing has been sent" />
      <BigStatus glyph="warn-filled" tone={colour.warn} amount="₦120,000"
      ...
      <Card style={{ gap: space.s5 }}>
        ...
        <Pressable accessibilityRole="button" onPress={() => setTyped(want)}>
          <View style={{ flexDirection: 'row', backgroundColor: colour.surface, borderRadius: 12, padding: space.s3 }}>
            <Row>{typed}</Row>
            <Row tone="tertiary">{want.slice(typed.length)}</Row>
          </View>
        </Pressable>
        <Caption tone="secondary">
          {left > 0 ? `${left} letters to go. Exactly those three words, nothing shorter.` : 'That is it. It can go now.'}
        </Caption>
      </Card>
      <Choices>
        <ChoiceRow glyph="send" title="Send ₦100,000 instead" sub="The rest tomorrow, no typing" onPress={() => nav.go('confirm')} />
      </Choices>
    </Screen>
  );
};
...
export const Devices = ({ nav }: { nav: Nav }) => (
  <Screen dock={dock('Ask about a device', nav)}>
    ...
    <AgentSay>The Windows one signed in from Abuja on 12 August and has not been back. If that was not you, sign it out.</AgentSay>
    <Button label="Sign out everywhere else" tone="grey" onPress={() => nav.go('settings')} />
    <Aside>Signing a device out never touches your money. It only asks whoever holds it to prove who they are again.</Aside>
  </Screen>
);
...
export const LostPhone = ({ nav }: { nav: Nav }) => (
  <Screen dock={dock('Ask what freezing does', nav)}>
    ...
    <View style={{ gap: 32 }}>
      <DeviceRow glyph="airtime" title="Freeze the money" where="Nothing can leave" tag="Do this"
        onPress={() => { update((s: { frozen?: boolean }) => { s.frozen = true; }); nav.go('newcode'); }} />
    ...
    <AgentSay>You are on a device this account has never seen. I will not send anything until somebody proves who they are.</AgentSay>
    <Button label="Freeze it, then prove it is me" onPress={() => nav.go('newcode')} />
    <Aside>Freezing stops money leaving. It does not stop money reaching you, and it does not sign you out here.</Aside>
  </Screen>
);
